...
class AudioDBManager: BaseDBManager {
    ...
    override func fetchDataWithLimit(childId: String, offset: Int, limit: Int, bundle: AndroidBundle) -> [BaseModel]? {
        ...
        do {
            ...
            audiocategorylist = getModelArrayFromFetchedResult(result: result)
        } catch let error as NSError {
    ...
    override func fetchAll() -> [BaseModel]? {
        ...
        do {
            ...
            audiocategorylist = getModelArrayFromFetchedResult(result: result as! [NSManagedObject])
        } catch let error as NSError {
            print("Could not fetch \(error), \(error.userInfo)")
        }
        return audiocategorylist
    ...
    private func getModelArrayFromFetchedResult(result:[NSManagedObject]) -> [BaseModel] {
        ...
        var audiocategorylist = [AudioCategoryModel]()
        for i in stride(from: 0, to: result.count, by: 1) {
            let row = result[i]
            //  print(row)
            let audioModel:AudioCategoryModel = getAudioCategoryModel(row: row)
            audiocategorylist.append(audioModel)
        }
        return audiocategorylist
    }
    
    private func getAudioCategoryModel(row:NSManagedObject) -> AudioCategoryModel {
        let audioCategoryModel = AudioCategoryModel()
        
        audioCategoryModel.categoryId = row.value(forKey: CAT_ID) as! String
        audioCategoryModel.categoryName = row.value(forKey: CAT_NAME) as! String
        audioCategoryModel.parentCategoryId = row.value(forKey: PCAT_ID) as! String
        audioCategoryModel.parentCategoryName = row.value(forKey: PCAT_NAME) as! String
        audioCategoryModel.imagePath = row.value(forKey: CAT_THUMB) as! String
        audioCategoryModel.isCategoryBlocked = row.value(forKey: BLOCKED_STATUS) as! String
        audioCategoryModel.canonicalName = row.value(forKey: CANONICAL_NAME) as! String
        audioCategoryModel.isVisible = row.value(forKey: VISIBILITY_STATUS) as! String
        audioCategoryModel.contentCount = row.value(forKey: CONTENT_COUNT) as! String
